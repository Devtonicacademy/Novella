...
export const BookCover: React.FC<BookCoverProps> = ({
  ...
  const { bgGradient = 'from-amber-950 via-zinc-900 to-black', accent = '#d97706', text = '#fef3c7', border = '#78350f' } = story.coverColorTheme || {};

  return (
    <div
      className={`relative rounded-xl overflow-hidden shadow-xl flex flex-col justify-between select-none transition-all duration-300 bg-gradient-to-b ${bgGradient} ${sizeClasses[size]} border border-zinc-800`}
      style={{
    ...
    >
      ...
      {story.coverImage && (
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className={`absolute inset-0 bg-gradient-to-t ${bgGradient} opacity-85`} />
        </div>
      )}
      ...
      <div className="relative z-10 flex items-center justify-between">
        <span
          ...
          style={{ color: text }}
        >
          Vol. {story.order < 10 ? `0${story.order}` : story.order}
        </span>
      ...
      <div className="relative z-10 my-auto text-center space-y-1.5 px-1">
        <div className="w-6 h-0.5 mx-auto rounded-full" style={{ backgroundColor: accent }} />
        <h4
      ...
      <div className="relative z-10 pt-1 border-t border-white/10 flex items-center justify-between text-[9px] text-zinc-400">
        <span className="truncate max-w-[80%]" style={{ color: text }}>
          {story.author}
        </span>
        <BookOpen className="w-3 h-3 opacity-60 shrink-0" style={{ color: text }} />
      </div>
    </div>
  );
};
